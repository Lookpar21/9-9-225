...
if (manualToggle){
  ...
  bind("mMain_Undo", ()=>{ manual.main.pop(); saveManual(); render(); });
  bind("mMain_Clear", ()=>{ manual.main = []; saveManual(); render(); });

  bind("mBE_Undo", ()=>{ manual.bigEye.pop(); saveManual(); render(); });
  bind("mBE_Clear", ()=>{ manual.bigEye = []; saveManual(); render(); });

  bind("mSM_Undo", ()=>{ manual.small.pop(); saveManual(); render(); });
  bind("mSM_Clear", ()=>{ manual.small = []; saveManual(); render(); });

  bind("mCR_Undo", ()=>{ manual.roach.pop(); saveManual(); render(); });
  bind("mCR_Clear", ()=>{ manual.roach = []; saveManual(); render(); });
}
